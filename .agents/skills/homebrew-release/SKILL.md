...
| --- | --- |
| macOS Apple Silicon | `thread-gateway-<version>-darwin-arm64.tar.gz` |
| macOS Intel | `thread-gateway-<version>-darwin-x64.tar.gz` |
...
```bash
version="$(tr -d '[:space:]' < VERSION)"
scripts/render-homebrew-formula.sh "$version" /path/to/homebrew-tap/Formula/thread-gateway.rb
```
...
brew install user/tap/thread-gateway
thread-gateway --version
brew test user/tap/thread-gateway
```
...
verify `api/formula/thread-gateway.json` from
