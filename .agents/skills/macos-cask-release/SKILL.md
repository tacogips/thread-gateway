...
```bash
brew tap user/tap
brew install --cask thread-gateway
```
...
swift run thread-gateway --version | tail -n 1 | grep -Fx "$version"
```
...
```bash
version="$(tr -d '[:space:]' < VERSION)"
/Applications/Xcode.app/Contents/Developer/usr/bin/stapler validate "dist/homebrew-cask/thread-gateway-${version}-darwin-arm64.dmg"
/Applications/Xcode.app/Contents/Developer/usr/bin/stapler validate "dist/homebrew-cask/thread-gateway-${version}-darwin-x64.dmg"
spctl --assess --type open --context context:primary-signature --verbose=4 "dist/homebrew-cask/thread-gateway-${version}-darwin-arm64.dmg"
spctl --assess --type open --context context:primary-signature --verbose=4 "dist/homebrew-cask/thread-gateway-${version}-darwin-x64.dmg"
```
...
`../homebrew-tap/Casks/thread-gateway.rb`.

After reviewing the rendered tap Cask:

```bash
cd ../homebrew-tap
git add Casks/thread-gateway.rb README.md
git diff --staged --stat
git commit -m "chore: release thread-gateway <version>"
...
verify `api/cask/thread-gateway.json` from GitHub
