...
cask "thread-gateway" do
  ...
  binary "thread-gateway-reader"
  binary "thread-gateway-writer"

  caveats do
    <<~EOS
      This cask installs the signed and notarized macOS command line tool.
      Homebrew links the reader and writer executables into the native Homebrew prefix for this Mac.
    EOS
  end
end
EOF
