...
products=("thread-gateway-reader" "thread-gateway-writer")
artifact_name="thread-gateway"
...
print_plan() {
  local version target release_dir work_dir dmg_path triple install_prefix
  version="$1"
  target="$2"
  release_dir="$3"
  ...
  printf '  staged signed binaries: %s/{%s}\n' "$work_dir" "${products[*]}"
...
build_target() {
  local version target release_dir work_dir dmg_path bin_path notarytool stapler product staged_binary
  version="$1"
  target="$2"
  release_dir="$3"
  work_dir="$release_dir/work/$artifact_name-$version-$target"
