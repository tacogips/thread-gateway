...
class App < Formula
  ...
  def install
    bin.install "bin/thread-gateway-reader", "bin/thread-gateway-writer"
  end

  test do
    assert_match "$version", shell_output("#{bin}/thread-gateway-reader --version")
    assert_match "$version", shell_output("#{bin}/thread-gateway-writer --version")
  end
end
EOF
